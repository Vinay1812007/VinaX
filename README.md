...
## What's new in 7.0 — smarter, safer, steadier

An audit-first release across the whole product. The **next-song engine** is now one explicit, traced pipeline (candidate generation → hard filtering → feature extraction → context scoring → diversity and repeat penalties → session adjustment → exploration tuning → ranking → sequencing → validation); whatever ordered the queue — the on-device sequencer or the AI DJ — passes the same final rules, and the on-device order ships whenever AI is slow, down or wrong. **Familiar / Balanced / Discover** (Settings → Recommendations) really changes ranking, the queue's share of never-played artists and the language policy. A new **session intent** layer follows this sitting's skips, completions, likes, searches and hand queue-adds without writing any of it into long-term taste. **Hand-queued songs outrank automation**, one song is one slot across versions (Unicode-safe), a play counts after five seconds and a skip takes its play back. Also: a critical Worker fix (clients could forge system events), data-safe restore and merge, a long list of playback, casting, sleep-timer and Android fixes, search that understands “song + singer” and Indic scripts, accessibility and safe-area work across every screen, and large render-performance wins. Developer score breakdowns (`?debug=recs`) show selected, passed-over and rejected songs with reasons. See [UPGRADE_7_0.md](UPGRADE_7_0.md).

## What's new in 6.5 — the DJ takes the wheel

The 3.9 listening behaviours, rebuilt on the 6.x engine: **tap any song and the DJ builds the rest** (setting “DJ builds every queue”, default on; Queue Builder plans and hand-queued songs are never replaced), **Tune this queue** (twelve one-tap intents on the Queue page, in the player’s extras and the command palette), a **generative DJ** that may propose a few songs beyond the local pool each round — each one verified in the catalogue by title and artist, language-locked and gated before it can play — with a rotating candidate slice and creative focus per round, **“Designed for you” rebuilt on every Home open** (idea engines pitch, the DJ engine curates, on-taste fallback shelves when engines are slow, page rotation and unseen-first per shelf), and Playlist Studio examples that build straight away. See [UPGRADE_6_5.md](UPGRADE_6_5.md).
...
**Familiar** brings favourites and finished songs back into the pool, ranks known songs and artists higher and keeps never-played artists to about one slot in twenty. **Balanced** is neutral in the score and gives never-played artists about a fifth of a queue; what is skipped and finished in a sitting tips it either way. **Discover** ranks never-played artists higher, gives them close to half a queue, lets the queue take a short detour into another language the listener plays, and adds unheard-language picks on Home. These controls guide recommendations; they do not promise a particular song or change tracks already queued. Hand-queued songs always go ahead of automatic picks.
