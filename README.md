...
- **Admin console:** `https://admin.sirimillavinay.online` (SPA served from `frontend/public/admin`, API at `/api/admin/*`, gated by `ADMIN_LOGIN_PASSWORD`); 26 tools organized under eight formal categories with collapsible sidebar groups (v5.7.5); the listener sidebar carries an Ads page whose sponsored placement loads its ad script ONLY on that page, with /ads.txt proving site ownership (v5.7.7).
