...
| `frontend/` | `npm test` | Vitest (507 tests / 74 files) |
...
|---|---|---|---|
| Frontend unit/component | `frontend/src/**/*.test.ts(x)` | 472 tests / 71 files | ✅ |
| Backend endpoint/lib | `backend/worker/**/*.test.ts` | 207 tests / 28 files | ✅ |
| Contracts | contrast + theme tokens, CSP hashes, festival artefact sync, router coverage, bundle budget | — | ✅ |
| E2E | `frontend/e2e/*.spec.ts` against the built bundle (external network aborted): admin console (every panel), VinaX AI, festival skins, the 5.17 feature set — 16 tests | `npm run e2e` | `e2e.yml` |
