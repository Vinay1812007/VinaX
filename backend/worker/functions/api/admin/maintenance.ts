/** Admin maintenance: destructive database actions, each explicit + audited
...
import { dbDelete, dbDeleteReturning, dbInsert, dbUpdate, type DbEnv } from '../../_lib/db';
...
export const onRequestPost = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  if (action === 'delete_user') {
    ...
    const okEvents = await dbDelete(env, 'vinax_events', `device_id=eq.${encodeURIComponent(id)}`);
    // Returning delete so a typo'd / already-deleted device_id is an honest
    // not_found instead of a silent `{ok:true}` no-op (D-18).
    const removed = await dbDeleteReturning<{ device_id: string }>(
      env,
      'vinax_users',
    ...
    await dbDelete(env, 'vinax_room_members', `device_id=eq.${encodeURIComponent(id)}`);
    ...
    await dbUpdate(env, 'vinax_feedback', `device_id=eq.${encodeURIComponent(id)}`, { device_id: 'deleted' });
  ...
  if (action === 'purge_events') {
    ...
    const ok = await dbDelete(env, 'vinax_events', `created_at=lt.${encodeURIComponent(cutoff)}`);
    if (ok) await logAdminAudit(env, 'maintenance', `Purged events older than ${days}d`);
    return json({ ok });
  }
  if (action === 'clear_errors') {
    const ok = await dbDelete(env, 'vinax_events', 'type=eq.error');
    if (ok) await logAdminAudit(env, 'maintenance', 'Cleared error events');
    return json({ ok });
  }
  if (action === 'trim_ai') {
    if (!days) return json({ error: 'bad_request' }, 400);
    const ok = await dbDelete(env, 'vinax_ai_events', `created_at=lt.${encodeURIComponent(cutoff)}`);
  ...
  if (action === 'clear_feedback') {
    ...
    const ok = await dbDelete(env, 'vinax_feedback', 'status=eq.resolved&type=neq.admin-audit');
  ...
  if (action === 'close_rooms') {
    ...
    const a = await dbDelete(env, 'vinax_room_members', 'last_seen=gte.1970-01-01');
    const b = await dbDelete(env, 'vinax_rooms', ALL);
  ...
  if (action === 'end_room') {
    ...
    const a = await dbDelete(env, 'vinax_room_members', `code=eq.${encodeURIComponent(code)}`);
    const b = await dbDelete(env, 'vinax_rooms', `code=eq.${encodeURIComponent(code)}`);
