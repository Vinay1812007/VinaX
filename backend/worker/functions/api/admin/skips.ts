 ...
 *     { configured, days, items:[{id,title,artist,image,plays,skips,rate}] }
...
export function skipTable(rows: Row[], min: number): Array<{ id: string; title: string; artist: string; image: string; plays: number; skips: number; rate: number }> {
...
export const onRequestGet = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  return json({ configured: true, days, min, sampled: rows.length, items: skipTable(rows, min) });
};
