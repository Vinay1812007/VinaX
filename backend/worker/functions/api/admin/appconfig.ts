 ...
 * key→json table (vinax_config, auto-created by _lib/db.ts) with an allowlist
...
export const ALLOWED_KEYS = new Set(['banners', 'home-config']);
...
export const onRequestGet = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  if (!dbConfigured(env)) return json({ configured: false, value: null });
  const key = new URL(request.url).searchParams.get('key') ?? '';
  if (!ALLOWED_KEYS.has(key)) return json({ error: 'unknown_key' }, 400);
  const rows = await dbSelect<ConfigRow>(env, 'vinax_config', `key=eq.${encodeURIComponent(key)}&select=key,value,updated_at&limit=1`).catch(
    () => [] as ConfigRow[],
  );
  const row = rows[0];
...
export const onRequestPost = async (context: { request: Request; env: Env }): Promise<Response> => {
  ...
  const ok = await dbUpsert(env, 'vinax_config', { key, value: body.value, updated_at: new Date().toISOString() }, 'key');
