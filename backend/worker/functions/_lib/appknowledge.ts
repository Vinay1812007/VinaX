...
- VinaX AI: a full chat with nineteen engines — the engine picker sits just above the message composer, and two of those engines open a further list of every free model they can run right now — plus live voice via the mic button in the composer, a Think toggle for harder reasoning and a Research toggle for web-checked answers.
...
- Make it yours: Settings → Home layout hides or reorders every big Home block (shelves, moods, charts, the endless feed). Settings → Appearance has two glass dials — Glass effect (solid to deep glass) and Background blur (sharp to hazy) — plus ten accent colours. On Android, tapping the playback notification opens the full-screen player.
- Notifications: opt-in; the daily song pick is personalized per listener — a song from their own language, announced in that language.
