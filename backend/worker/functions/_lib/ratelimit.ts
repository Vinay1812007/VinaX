 ...
 *
 * H-SRV-11 (resolved): sensitive routes opt into `global: true`, which
 * consults the RateLimiterDO Durable Object (one instance per route) for a
 * globally consistent bucket — colo-sharding no longer resets the budget.
 * The map key is hashed with a pepper so raw client IPs never sit in memory
 * even for the isolate's lifetime.
 */
interface Bucket {
  tokens: number;
  last: number;
 ...
 *  existing `const limited = await rateLimit(...)` shape. */
...
// Structural type for the RATE_LIMIT Durable Object binding (no SDK dep).
interface DoStub {
...
function limited429(retryAfterSec: number): Response {
 ...
 *
 * Two tiers (H-SRV-11): the per-isolate memory bucket always runs (free,
 * zero-latency, blunts floods at every PoP). Routes that opt into
 * `global: true` ALSO consult the RateLimiterDO — one Durable Object per
 * route, globally consistent, so colo-sharding no longer resets the budget.
 * If the DO binding is absent or the call fails, we fail open to the memory
 * verdict; a broken limiter must never take down a working endpoint.
 */
export async function rateLimit(
  request: Request,
  route: string,
  opts: { capacity?: number; refillPerMinute?: number; global?: boolean } = {},
  env?: PepperEnv,
  ...
  if (opts.global) {
    const ns =
      env && typeof env === 'object' ? (env as { RATE_LIMIT?: DoNamespace }).RATE_LIMIT : undefined;
    if (ns && typeof ns.idFromName === 'function') {
      try {
        const stub = ns.get(ns.idFromName(route));
        const res = await stub.fetch('https://ratelimit.do/check', {
          method: 'POST',
          body: JSON.stringify({ key: ipKey, capacity, refillPerMinute: opts.refillPerMinute ?? 10 }),
        });
        const verdict = (await res.json()) as { allow?: boolean; retryAfter?: number };
        if (verdict.allow === false) return limited429(Math.max(1, Number(verdict.retryAfter) || 1));
        return null; // global bucket allowed it; skip the per-isolate bucket
      } catch {
        /* DO unavailable — fall through to the per-isolate bucket */
      }
    }
  }
  ...
  return limited429(Math.max(1, Math.ceil((1 - b.tokens) / refillPerMs / 1000)));
}
