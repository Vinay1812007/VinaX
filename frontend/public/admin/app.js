(function () {
  ...
  function renderHomeBuilderSection() {
    ...
    cfgGet('home-layout').then(function (d) {
      ...
      order = order.concat(defaults.order.filter(function (x) { return order.indexOf(x) < 0; }));
      $('view').innerHTML = '<div class="card"><h3 style="margin-top:0">Home Layout Studio</h3><p class="muted">Set the production default for the listener Home. Changes are validated by the public config endpoint.</p>' + lbl('Headline', inp('home-title', value.title || defaults.title, '', 'style="width:100%;max-width:640px" maxlength="60"')) + lbl('Description', '<textarea id="home-desc" class="inp" maxlength="160" style="width:100%;max-width:640px;min-height:70px">' + esc(value.description || defaults.description) + '</textarea>') + '<h4>Visible shelf order</h4><div id="home-order">' + order.map(function (key, i) { return '<div class="row home-admin-row" data-key="' + esc(key) + '" style="gap:8px;padding:8px 0;border-bottom:1px solid var(--border)"><code style="width:24px">' + String(i + 1).padStart(2, '0') + '</code><b style="flex:1">' + esc(key) + '</b><button class="ghost" data-home-up="' + esc(key) + '"' + (i === 0 ? ' disabled' : '') + '>↑</button><button class="ghost" data-home-down="' + esc(key) + '"' + (i === order.length - 1 ? ' disabled' : '') + '>↓</button></div>'; }).join('') + '</div><div class="row" style="gap:8px;margin-top:16px"><button id="home-save">Publish layout</button><button class="ghost" id="home-reset">Restore balanced</button><span class="muted" id="home-msg"></span></div></div>';
      document.querySelectorAll('[data-home-up],[data-home-down]').forEach(function (b) { b.addEventListener('click', function () { var key = b.getAttribute('data-home-up') || b.getAttribute('data-home-down'), step = b.hasAttribute('data-home-up') ? -1 : 1, next = order.slice(), i = next.indexOf(key); if (i < 0 || !next[i + step]) return; next[i] = next[i + step]; next[i + step] = key; cfgSet('home-layout', Object.assign({}, value, { order: next })).then(function () { renderHomeBuilderSection(); }); }); });
      $('home-save').addEventListener('click', function () { cfgSet('home-layout', { title: $('home-title').value.trim(), description: $('home-desc').value.trim(), order: order, hidden: [] }).then(function (r) { $('home-msg').textContent = r && r.ok ? 'Published ✓' : 'Could not publish'; }); });
      $('home-reset').addEventListener('click', function () { cfgSet('home-layout', defaults).then(function (r) { if (r && r.ok) renderHomeBuilderSection(); }); });
      stamp();
    }).catch(function () { showFail('Could not load Home layout.'); });
  }
  // 22. Help center FAQ.
  ...
  function refreshActive() {
    ...
    else if (active === 'homebuilder') renderHomeBuilderSection();
