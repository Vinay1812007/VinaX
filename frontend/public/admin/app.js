(function () {
  ...
  function renderHomeBuilderSection() {
    ...
    cfgGet('home-layout').then(function (d) {
      ...
      $('home-save').addEventListener('click', function () { var h = currentHidden(); if (h.length >= order.length) { $('home-msg').textContent = 'At least one shelf must stay on.'; return; } cfgSet('home-layout', { title: $('home-title').value.trim(), description: $('home-desc').value.trim(), order: order, hidden: h }).then(function (r) { $('home-msg').textContent = r && r.ok ? 'Published ✓ · reaches listeners within about five minutes' : 'Could not publish'; }); });
